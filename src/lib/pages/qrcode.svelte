...
<div class="px-6 py-8 max-w-5xl mx-auto">
		<header class="mb-8">
			<h1 class="text-2xl font-bold text-foreground">{$_('features.qrcode.title')}</h1>
		...
		<div class="mb-6">
			<Tab options={[
				{ value: 'generate', label: $_('qrcode.generate'), icon: QrCode },
				{ value: 'scan', label: $_('qrcode.scan'), icon: ScanLine }
			]} bind:value={mode} size="default" />
		</div>
					...
					<Card.Content>
						...
						<div class="flex flex-wrap gap-4 mt-4">
							<div class="flex items-center gap-2">
								...
								<Tab options={[128, 256, 512, 1024].map(v => ({ value: v, label: `${v}px` }))} bind:value={qrSize} onchange={() => generateQR()} />
							</div>
							<div class="flex items-center gap-2">
								<span class="text-sm text-slate-500">{$_('qrcode.errorLevel')} <HintIcon text={$_('qrcode.errorLevelHint')} /></span>
								<Tab options={['L', 'M', 'Q', 'H'].map(v => ({ value: v, label: v }))} bind:value={errorLevel} onchange={() => generateQR()} />
							</div>
							<div class="flex items-center gap-2">
								...
								]} bind:value={outputFormat} onchange={() => generateQR()} />
							</div>
						</div>
						...
						<div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
							...
							<div class="flex flex-wrap gap-2 mb-3">
								{#each presetColors as preset}
									<button
										class="w-7 h-7 rounded-md border-2 transition-all {darkColor === preset.dark && lightColor === preset.light ? 'border-primary ring-2 ring-primary/30' : 'border-border hover:border-foreground/30'}"
							...
							<div class="flex flex-wrap gap-4">
								...
								<div class="flex items-center gap-2">
									...
									<Tab options={[0, 1, 2, 3, 4].map(v => ({ value: v, label: String(v) }))} bind:value={margin} onchange={() => generateQR()} />
								</div>
							</div>
							...
							<div class="mb-4">
								...
								<div class="flex flex-wrap gap-1">
									{#each dotStyles as style}
										<button
											class="flex flex-col items-center p-1 rounded-lg border-2 transition-all {dotStyle === style.value ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30' : 'border-border hover:border-foreground/30'}"
							...
							<div class="mb-4">
								...
								<div class="flex flex-wrap gap-1">
									{#each finderStyles as style}
										<button
											class="flex flex-col items-center p-1 rounded-lg border-2 transition-all {finderStyle === style.value ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30' : 'border-border hover:border-foreground/30'}"
