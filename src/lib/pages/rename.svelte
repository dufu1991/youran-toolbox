<script lang="ts">
	...
	import { stat, rename, copyFile, mkdir, readFile } from '@tauri-apps/plugin-fs';
	...
	import { ChevronLeft, FolderUp, X, Plus, Trash2, ArrowRight, Eye, EyeOff, Copy, Pencil, FolderOpen, Play, Hash, Type, Calendar, HardDrive, FileText, Minus, Clock, FileClock, Shuffle, Image as ImageIcon } from 'lucide-svelte';
	...
	type RuleType = 'index' | 'text' | 'date' | 'createdDate' | 'modifiedDate' | 'size' | 'pixel' | 'name' | 'separator' | 'random';
	...
	const ruleTypeConfig: Record<RuleType, { color: string; bgColor: string; borderColor: string; icon: typeof Hash }> = {
		...
		pixel: { color: 'text-rose-600 dark:text-rose-400', bgColor: 'bg-rose-50 dark:bg-rose-900/30', borderColor: 'border-rose-200 dark:border-rose-700', icon: ImageIcon },
	...
	const allImages = $derived(files.length > 0 && files.every(f => f.width !== undefined));
	...
	// 图片扩展名
	const IMAGE_EXTS = new Set(['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'tiff', 'tif', 'avif', 'ico']);
	...
	function getImageDimensions(data: Uint8Array, name: string): Promise<{ width: number; height: number }> {
		return new Promise((resolve) => {
			const ext = name.split('.').pop()!.toLowerCase();
			const mimeMap: Record<string, string> = { jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', gif: 'image/gif', webp: 'image/webp', bmp: 'image/bmp', tiff: 'image/tiff', tif: 'image/tiff', avif: 'image/avif', ico: 'image/x-icon' };
			const blob = new Blob([new Uint8Array(data)], { type: mimeMap[ext] || 'image/*' });
	...
	function formatPixel(width: number | undefined, height: number | undefined, mode: PixelMode, separator: PixelSeparator = 'x', ratioSep: RatioSeparator = '-'): string {
		const w = width ?? 0;
		const h = height ?? 0;
		if (mode === 'width') return `${w}`;
		if (mode === 'height') return `${h}`;
		if (mode === 'ratio') {
			if (w === 0 || h === 0) return `0${ratioSep}0`;
			const d = gcd(w, h);
			return `${w / d}${ratioSep}${h / d}`;
		}
	...
	function getRulePreview(rule: RenameRule, file?: FileItem): string {
		...
		if (rule.type === 'pixel') {
			...
			return formatPixel(1920, 1080, rule.pixelMode, rule.pixelSeparator, rule.ratioSeparator);
		}
		if (rule.type === 'name') {
...
<div class="px-6 py-8 max-w-5xl mx-auto">
		<header class="mb-8">
			<h1 class="text-2xl font-bold text-foreground">{$_('features.rename.title')}</h1>
				...
				<Card.Header class="pb-3">
					<div class="flex items-center justify-between flex-wrap gap-2">
						...
						<div class="flex items-center gap-2 flex-wrap">
							...
							{#if allImages}
							<button
								class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-md border transition-colors bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 border-rose-200 dark:border-rose-700 hover:bg-rose-100 dark:hover:bg-rose-900/50"
								onclick={() => addRuleOfType('pixel')}
							>
							...
							{#if editingRule}
								...
								<div class="mt-3 p-4 rounded-lg border {config.bgColor} {config.borderColor}">
									...
									{#if editingRule.type === 'index'}
										<div class="flex items-center gap-2">
											...
											<NumberStepper bind:value={editingRule.indexPadding} min={1} max={10} />
										</div>
										...
										<div class="space-y-3">
											<div>
												...
												<div class="flex gap-2 flex-wrap">
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'year' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
														onclick={() => editingRule.datePrecision = 'year'}
													>{$_('rename.dateYear')}</button>
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'month' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
														onclick={() => editingRule.datePrecision = 'month'}
													>{$_('rename.dateMonth')}</button>
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'day' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
														onclick={() => editingRule.datePrecision = 'day'}
													>{$_('rename.dateDay')}</button>
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'hour' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
														onclick={() => editingRule.datePrecision = 'hour'}
													>{$_('rename.dateHour')}</button>
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'minute' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
														onclick={() => editingRule.datePrecision = 'minute'}
													>{$_('rename.dateMinute')}</button>
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'second' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
														onclick={() => editingRule.datePrecision = 'second'}
													>{$_('rename.dateSecond')}</button>
													<span class="text-slate-300 dark:text-slate-600">|</span>
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'monthDay' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
														onclick={() => editingRule.datePrecision = 'monthDay'}
													>{$_('rename.dateMonthDay')}</button>
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'dayOnly' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
														onclick={() => editingRule.datePrecision = 'dayOnly'}
													>{$_('rename.dateDayOnly')}</button>
													<span class="text-slate-300 dark:text-slate-600">|</span>
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'hourOnly' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
														onclick={() => editingRule.datePrecision = 'hourOnly'}
													>{$_('rename.dateHourOnly')}</button>
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'hourMinute' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
														onclick={() => editingRule.datePrecision = 'hourMinute'}
													>{$_('rename.dateHourMinute')}</button>
													<button
														class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.datePrecision === 'time' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
									...
									<div>
										...
										<div class="flex gap-2 flex-wrap">
											<button
												class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.dateSeparator === 'dash' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
												onclick={() => editingRule.dateSeparator = 'dash'}
											>2024-01-30</button>
											<button
												class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.dateSeparator === 'underscore' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
												onclick={() => editingRule.dateSeparator = 'underscore'}
											>2024_01_30</button>
											<button
												class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.dateSeparator === 'none' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
												onclick={() => editingRule.dateSeparator = 'none'}
											>20240130</button>
											<button
												class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.dateSeparator === 'chinese' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
								...
								{#if editingRule.dateSeparator !== 'chinese' && ['hour', 'minute', 'second', 'time', 'hourMinute'].includes(editingRule.datePrecision)}
									<div>
										...
										<div class="flex gap-2 flex-wrap">
											<button
												class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.dateTimeSeparator === 'underscore' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
												onclick={() => editingRule.dateTimeSeparator = 'underscore'}
											>日期_时间</button>
											<button
												class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.dateTimeSeparator === 'dash' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
												onclick={() => editingRule.dateTimeSeparator = 'dash'}
											>日期-时间</button>
											<button
												class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.dateTimeSeparator === 'none' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
									...
									<div>
										...
										<div class="flex gap-2 flex-wrap">
											<button
												class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.timeSeparator === 'dot' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
												onclick={() => editingRule.timeSeparator = 'dot'}
											>10.30.00</button>
											<button
												class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.timeSeparator === 'dash' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
												onclick={() => editingRule.timeSeparator = 'dash'}
											>10-30-00</button>
											<button
												class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.timeSeparator === 'none' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
								...
								{:else if editingRule.type === 'size'}
									<div class="space-y-3">
										<div>
											...
											<div class="flex gap-2 flex-wrap">
												<button
													class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.sizeUnit === 'auto' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
													onclick={() => editingRule.sizeUnit = 'auto'}
												>{$_('rename.sizeAuto')}</button>
												<button
													class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.sizeUnit === 'B' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
													onclick={() => editingRule.sizeUnit = 'B'}
												>B</button>
												<button
													class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.sizeUnit === 'KB' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
													onclick={() => editingRule.sizeUnit = 'KB'}
												>KB</button>
												<button
													class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.sizeUnit === 'MB' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
													onclick={() => editingRule.sizeUnit = 'MB'}
												>MB</button>
												<button
													class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.sizeUnit === 'GB' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
													onclick={() => editingRule.sizeUnit = 'GB'}
												>GB</button>
												<button
													class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.sizeUnit === 'TB' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
										...
										<div class="flex items-center gap-3">
											...
											<Switch size="sm" checked={editingRule.sizeShowUnit} onchange={() => editingRule.sizeShowUnit = !editingRule.sizeShowUnit} />
										</div>
								...
								{:else if editingRule.type === 'pixel'}
						<div class="space-y-3">
							<div>
								<p class="text-xs text-slate-500 mb-2">{$_('rename.pixelMode')}</p>
								<div class="flex gap-2 flex-wrap">
									<button
										class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.pixelMode === 'both' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
										onclick={() => editingRule.pixelMode = 'both'}
									>{$_('rename.pixelBoth')}</button>
									<button
										class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.pixelMode === 'width' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
										onclick={() => editingRule.pixelMode = 'width'}
									>{$_('rename.pixelWidth')}</button>
									<button
										class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.pixelMode === 'height' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
										onclick={() => editingRule.pixelMode = 'height'}
									>{$_('rename.pixelHeight')}</button>
								<button
									class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.pixelMode === 'ratio' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
									onclick={() => editingRule.pixelMode = 'ratio'}
								>{$_('rename.pixelRatio')}</button>
								</div>
							</div>
							{#if editingRule.pixelMode === 'both'}
								<div>
							...
							{/if}
							{#if editingRule.pixelMode === 'ratio'}
								<div>
							...
							{/if}
							<p class="text-xs text-slate-400">{$_('rename.datePreview')}: {formatPixel(1920, 1080, editingRule.pixelMode, editingRule.pixelSeparator, editingRule.ratioSeparator)}</p>
						</div>
				{:else if editingRule.type === 'name'}
									<div class="space-y-3">
										<div>
											...
											<div class="flex gap-2 flex-wrap">
												<button
													class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.nameOperation === 'full' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
													onclick={() => editingRule.nameOperation = 'full'}
												>{$_('rename.nameOpFull')}</button>
												<button
													class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.nameOperation === 'replace' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
													onclick={() => editingRule.nameOperation = 'replace'}
												>{$_('rename.nameOpReplace')}</button>
												<button
													class="px-3 py-1.5 text-sm rounded-md transition-colors {editingRule.nameOperation === 'slice' ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border'}"
								...
								{#if editingRule.nameOperation === 'replace'}
									<div class="space-y-2">
										...
										<button
											class="px-3 py-1.5 text-sm rounded-md bg-blue-500 text-white hover:bg-primary/90 transition-colors whitespace-nowrap"
											onclick={() => editingRuleId = null}
										>{$_('rename.nameOpReplace')}</button>
										</div>
								...
								{:else if editingRule.nameOperation === 'slice'}
									<div class="flex items-center gap-2">
										...
										<NumberStepper bind:value={editingRule.nameSliceStart} min={0} max={999} />
										<span class="text-xs text-slate-500">{$_('rename.nameSliceEnd')}</span>
										<NumberStepper bind:value={editingRule.nameSliceEnd} min={0} max={999} />
							...
							<div>
								...
								<div class="flex gap-2 flex-wrap">
									{#each separatorOptions as sep}
										<button
											class="w-10 h-10 text-lg font-mono rounded-md transition-colors {editingRule.value === sep ? 'bg-primary text-primary-foreground' : 'bg-transparent border border-border hover:bg-slate-100 dark:hover:bg-slate-700'}"
						...
						{:else if editingRule.type === 'random'}
							<div class="space-y-3">
								<div class="flex items-center gap-4">
									...
									<NumberStepper bind:value={editingRule.randomLength} min={1} max={32} />
								</div>
								<div>
				...
				<Card.Header class="pb-3">
					...
					<div class="flex flex-wrap items-center gap-4 mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
						<div class="flex items-center gap-2">
							...
							<Tab options={[
								{ value: 'name', label: $_('rename.sortName') },
								{ value: 'size', label: $_('rename.sortSize') },
								{ value: 'created', label: $_('rename.sortCreated') },
								{ value: 'modified', label: $_('rename.sortModified') }
							]} bind:value={sortBy} />
							{#if sortBy === 'name'}
								<div class="ml-2 pl-2 border-l border-slate-300 dark:border-slate-600">
									<Tab options={[
										{ value: false, label: $_('rename.sortOldName') },
										{ value: true, label: $_('rename.sortNewName') }
									]} bind:value={sortByNewName} />
								</div>
							{/if}
						</div>
						<div class="flex items-center gap-2">
							...
							<Tab options={[
								{ value: 'asc', label: $_('rename.asc') },
								{ value: 'desc', label: $_('rename.desc') }
					...
					<div class="flex flex-wrap items-center gap-6">
						...
						{#if copyMode}
							<div class="flex items-center gap-3">
								...
								<Tab options={[
									{ value: 'inplace', label: $_('rename.copyModeInplace') },
									{ value: 'archive', label: $_('rename.copyModeArchive') }
