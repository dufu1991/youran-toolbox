		...
		{#if hasData}
			<div class="flex flex-wrap items-center gap-x-6 gap-y-3 mb-6 p-4 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700">
				...
				<div class="flex items-center gap-2">
					...
					<Tab options={[0, 1, 2, 3, 4].map(v => ({ value: v, label: String(v) }))} bind:value={cellGap} />
				</div>
				<div class="flex items-center gap-2">
					<span class="text-sm text-slate-500">{$_('heatmap.xLabelAngle')}</span>
					<Tab options={[-90, -82.5, -75, -67.5, -60, -52.5, -45, -37.5, -30, -22.5, -15].map(v => ({ value: v, label: `${v}°` }))} bind:value={xLabelAngle} />
				</div>
			</div>
		{/if}
		...
		{#if heatmapData}
			...
			<div class="flex justify-end items-center gap-3 mt-4">
				...
				<div class="flex items-center gap-2">
					...
					<Tab options={[1, 2, 3].map(v => ({ value: v, label: `${v}x` }))} bind:value={pixelRatio} />
				</div>
